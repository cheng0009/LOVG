...
class VideoService:
    ...
    def _check_memory_usage(self, threshold_percent=80):
        """检查内存使用情况"""
        try:
            memory = psutil.virtual_memory()
            if memory.percent > threshold_percent:
                print(f"⚠️ 内存使用率过高: {memory.percent}%，等待释放...")
                time.sleep(5)  # 等待5秒
                return False
            return True
        except:
    ...
    def merge_video_clips(self, video_paths: List[str], output_filename: str) -> Optional[str]:
        ...
        try:
            # 检查内存使用情况
            if not self._check_memory_usage(75):
                print("⚠️ 内存压力较大，延迟处理...")
                time.sleep(10)
    ...
    def _merge_videos_direct(self, video_paths: List[str], output_filename: str) -> Optional[str]:
        ...
        # 限制同时处理的视频数量
        max_concurrent_clips = 1  # 进一步减少同时加载的视频数量
        processed_count = 0
        
        for i, path in enumerate(video_paths):
            try:
                # 如果已经处理了足够多的视频，先合并一部分
                if len(clips) >= max_concurrent_clips:
                    print(f"⚠️ 达到并发限制({max_concurrent_clips})，先处理当前批次")
                    temp_output = self._process_clip_batch(clips, f"{output_filename}_temp_{processed_count}")
                    if temp_output and Path(temp_output).exists():
                        # 重新开始新的批次
                        clips = [VideoFileClip(temp_output)]
                        processed_count += 1
                        # 强制垃圾回收
                        gc.collect()
                        time.sleep(2)
        ...
        # 最终处理所有片段
        result = self._process_clip_batch(clips, output_filename)
        
        # 清理中间文件
        for i in range(processed_count):
            temp_file = Config.FINAL_VIDEO_DIR / f"{output_filename}_temp_{i}.mp4"
            if temp_file.exists():
                try:
                    temp_file.unlink()
                    print(f"✅ 清理临时文件: {temp_file}")
                except:
                    print(f"⚠️ 无法清理临时文件: {temp_file}")
        
        return result
    
    def _process_clip_batch(self, clips: List[VideoFileClip], output_filename: str) -> Optional[str]:
        """处理一批视频片段"""
        try:
            if not clips:
                return None
            ...
            # 导出视频（使用原始帧率，避免强制转换）
            print(f"ℹ️ 开始导出视频，预计时长: {sum(c.duration for c in clips):.2f}s")
            ...
            final_video.write_videofile(
                ...
                audio_codec='aac',
                codec='libx264',
                bitrate='3000k',      # 降低码率以减少内存使用
                preset='ultrafast',   # 使用超快预设减少内存占用
                threads=1,           # 减少线程数
    ...
    def _merge_videos_batched(self, video_paths: List[str], output_filename: str, batch_size: int = 1) -> Optional[str]:
        """分批合并视频片段以降低内存使用"""
    ...
    def _add_subtitles_with_opencv(self, video_path: str, subtitles: List[Dict], output_filename: str, style_config: Dict = None) -> Optional[str]:
        ...
        try:
            ...
            if style_config:
                ...
                video_fps = video_clip.fps  # 获取视频帧率
            ...
            video_fps = clip.fps  # 确保获取正确的帧率
            
            # 计算帧时间间隔，用于更精确的时间匹配
            frame_duration = 1.0 / video_fps if video_fps > 0 else 0.033  # 默认30fps
