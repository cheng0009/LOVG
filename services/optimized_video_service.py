...
class OptimizedVideoService:
    ...
    def merge_video_clips_batch(self, video_paths: List[str], output_filename: str, batch_size: int = 3) -> Optional[str]:
        """分批合并视频片段以降低内存使用"""
        try:
            # 过滤掉无效的视频路径
            ...
            print(f"ℹ️ 开始分批合并 {len(valid_paths)} 个视频片段 (批大小: {batch_size})")
            ...
            if len(intermediate_videos) == 1:
                ...
                return self._merge_videos_direct(intermediate_videos, output_filename, is_intermediate=False)
            
        except Exception as e:
            print(f"分批合并视频失败: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
        finally:
            # 清理中间文件
            self._cleanup_intermediate_files(output_filename)
    
    def _merge_videos_direct(self, video_paths: List[str], output_filename: str, is_intermediate: bool = True) -> Optional[str]:
        """直接合并视频片段"""
        try:
            # 加载视频片段
            clips = []
            ...
            print(f"直接合并视频失败: {str(e)}")
            import traceback
            traceback.print_exc()
            return None
    
    def _cleanup_intermediate_files(self, base_filename: str):
        """清理中间文件"""
        try:
            # 查找并删除临时目录中的中间文件
            temp_pattern = f"{base_filename}_batch_*"
            for temp_file in Config.TEMP_DIR.glob(f"{temp_pattern}.mp4"):
                if temp_file.exists():
                    temp_file.unlink()
                    print(f"🗑️ 清理中间文件: {temp_file}")
        except Exception as e:
            print(f"清理中间文件失败: {str(e)}")
    
    def merge_video_clips(self, video_paths: List[str], output_filename: str) -> Optional[str]:
        """合并视频片段（兼容旧接口）"""
        # 默认使用分批处理，批大小为3以降低内存使用
        return self.merge_video_clips_batch(video_paths, output_filename, batch_size=3)
