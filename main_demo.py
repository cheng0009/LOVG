...
with st.sidebar:
    ...
    # 提示词模板编辑
    with st.expander("📝 编辑提示词模板"):
        # 脚本生成提示词
        st.markdown("**脚本生成提示词模板**")
        script_prompt = st.text_area(
            "脚本生成提示词",
            value=PromptsConfig.SCRIPT_GENERATION_PROMPT,
            height=300,
            help="这是发送给AI的完整提示词，支持变量: {topic}, {duration}, {scene_count}, {style}, {language}",
            key="script_prompt_edit"
        )
        
        if st.button("💾 保存脚本提示词"):
            PromptsConfig.SCRIPT_GENERATION_PROMPT = script_prompt
            st.success("脚本生成提示词已更新！")
        
        st.markdown("---")
        
        # 图像优化提示词
        st.markdown("**图像优化提示词模板**")
        image_system_prompt = st.text_area(
            "图像优化系统提示词",
            value=PromptsConfig.IMAGE_OPTIMIZATION_SYSTEM_PROMPT,
            height=200,
            key="image_system_prompt_edit"
        )
        
        if st.button("💾 保存图像优化提示词"):
            PromptsConfig.IMAGE_OPTIMIZATION_SYSTEM_PROMPT = image_system_prompt
            st.success("图像优化提示词已更新！")
        
        st.markdown("---")
        
        # 视频优化提示词
        st.markdown("**视频优化提示词模板**")
        video_system_prompt = st.text_area(
            "视频优化系统提示词",
            value=PromptsConfig.VIDEO_OPTIMIZATION_SYSTEM_PROMPT,
            height=200,
            key="video_system_prompt_edit"
        )
        
        if st.button("💾 保存视频优化提示词"):
            PromptsConfig.VIDEO_OPTIMIZATION_SYSTEM_PROMPT = video_system_prompt
            st.success("视频优化提示词已更新！")
    
    # 风格配置
    with st.expander("🎭 风格设置"):
        current_styles = list(PromptsConfig.STYLE_TEMPLATES.keys())
        selected_style = st.selectbox(
            "选择默认风格",
            options=current_styles,
            index=current_styles.index(PromptsConfig.DEFAULT_SETTINGS.get('style', '现代简约')) 
                  if PromptsConfig.DEFAULT_SETTINGS.get('style', '现代简约') in current_styles else 0
        )
        
        if st.button("💾 更新默认风格"):
            PromptsConfig.DEFAULT_SETTINGS['style'] = selected_style
            st.success(f"默认风格已更新为: {selected_style}")
    
    # 设置配置
    with st.expander("⚙️ 默认设置"):
        duration = st.slider("默认时长(秒)", 30, 180, 
                            PromptsConfig.DEFAULT_SETTINGS.get('duration', 60))
        scene_count = st.slider("默认分镜数", 3, 8, 
                               PromptsConfig.DEFAULT_SETTINGS.get('scene_count', 5))
        
        if st.button("💾 保存设置"):
            PromptsConfig.DEFAULT_SETTINGS.update({
                'duration': duration,
    ...
    with st.expander("📁 配置管理"):
        if st.button("💾 保存全部配置"):
            try:
                ...
                st.success("配置已保存到 user_prompts_config.json")
...
if st.session_state.step == 1:
    ...
    # 显示当前使用的配置
    st.info(f"""
    🎨 **当前配置**: 风格={PromptsConfig.DEFAULT_SETTINGS.get('style', '现代简约')} | 
    时长={PromptsConfig.DEFAULT_SETTINGS.get('duration', 60)}秒 | 
    分镜数={PromptsConfig.DEFAULT_SETTINGS.get('scene_count', 5)}张
    
    ✏️ 可在右侧“提示词配置”中修改默认设置
    """)
        ...
        if not topic.strip():
            ...
            with st.spinner("正在生成脚本..."):
                ...
                st.session_state.results['settings'] = {
                    'duration': PromptsConfig.DEFAULT_SETTINGS.get('duration', 60),
                    'scene_count': PromptsConfig.DEFAULT_SETTINGS.get('scene_count', 5),
                    'style': PromptsConfig.DEFAULT_SETTINGS.get('style', '现代简约'),
                    'language': PromptsConfig.DEFAULT_SETTINGS.get('language', '中文')
                }
                st.session_state.results['scripts'] = DEMO_SCRIPTS
                st.session_state.results['storyboard_prompts_list'] = DEMO_SCRIPTS['storyboard_prompts']
                st.success("脚本生成成功！")
